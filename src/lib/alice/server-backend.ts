...
// ===== Supabase Config (.env) =====
const _PROJECT_ROOT = process.env.ALICE_PROJECT_ROOT || process.cwd();

export const saveSupabaseConfig = createServerFn({ method: "POST" })
  .validator((d: { url?: string; anonKey?: string }) => d)
  .handler(async ({ data }) => {
    const fs = await import("node:fs");
    const envPath = path.join(_PROJECT_ROOT, ".env");
    let envContent = "";
    try {
      envContent = fs.readFileSync(envPath, "utf-8");
    } catch {
      // .env doesn't exist, create it
      envContent = "# Alice AI Agent - Environment Variables\n# Auto-generated by Alice Settings UI\n\n";
    }
    ...
    if (data.url) {
      envVars["SUPABASE_URL"] = data.url;
      envVars["VITE_SUPABASE_URL"] = data.url;
    }
    if (data.anonKey) {
      envVars["SUPABASE_PUBLISHABLE_KEY"] = data.anonKey;
      envVars["VITE_SUPABASE_PUBLISHABLE_KEY"] = data.anonKey;
    }
    // Rebuild .env file preserving comments
    const newLines: string[] = [];
    let addedSupabase = false;
    for (const line of lines) {
      const trimmed = line.trim();
      const key = trimmed.split("=")[0];
      if (key && (key.startsWith("SUPABASE") || key.startsWith("VITE_SUPABASE"))) {
        // Skip old Supabase lines, we'll add them below
        continue;
      }
      newLines.push(line);
    }
    // Find where to insert Supabase config (after first comment block or at end)
    const insertIdx = newLines.findIndex((l) => l.trim().startsWith("# ==="));
    const supabaseLines = [
      "# === Cloud Sync (Supabase) ===",
    ];
    if (data.url) supabaseLines.push(`SUPABASE_URL=${data.url}`);
    if (data.anonKey) supabaseLines.push(`SUPABASE_PUBLISHABLE_KEY=${data.anonKey}`);
    if (data.url) supabaseLines.push(`VITE_SUPABASE_URL=$SUPABASE_URL`);
    if (data.anonKey) supabaseLines.push(`VITE_SUPABASE_PUBLISHABLE_KEY=$SUPABASE_PUBLISHABLE_KEY`);
    supabaseLines.push("");
    if (insertIdx >= 0) {
      newLines.splice(insertIdx + 1, 0, "", ...supabaseLines);
    } else {
      newLines.push("", ...supabaseLines);
    }
    fs.writeFileSync(envPath, newLines.join("\n"), "utf-8");
    return { saved: true, envPath };
  });
